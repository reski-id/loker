  ...
  <tbody>
    ...
    foreach ($query->result() as $key => $value) {
      ...
      <tr>
        ...
        <td class="text-center">
          <a href="penerimaan/v/d/<?php echo hashids_encrypt($value->id_daftar_loker); ?>" class="btn btn-info btn-xs" title="Detail"><i class="fa fa-list"></i></a>
        <?php if ($level=='user' && $value->status==0){ ?>
          <a href="penerimaan/v/h/<?php echo hashids_encrypt($value->id_daftar_loker); ?>" class="btn btn-danger btn-xs" title="Batalkan" onclick="return confirm('Anda Yakin?');"><i class="fa fa-close"></i></a>
        <?php } ?>
        </td>
      </tr>
    <?php
